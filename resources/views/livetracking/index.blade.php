...
function getWorkerStatusBadge(status) {
    ...
    if (status === 'on_route') colorClasses = 'bg-yellow-100 text-yellow-700 border border-yellow-200';
    else if (status === 'on_site') colorClasses = 'bg-orange-100 text-orange-700 border border-orange-200';
    else if (status === 'completed') colorClasses = 'bg-green-100 text-green-700 border border-green-200';
        ...
        .then(jobOrders => {
            ...
            jobOrders.forEach(order => {
                ...
                order.workers.forEach(worker => {
                    ...
                    <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition duration-150">
                    ...
                    <span class="font-semibold text-gray-800 text-sm">${worker.name}</span>
                    ...
                    <button onclick="toggleAccordion(${order.id})" class="w-full px-6 py-5 flex flex-col sm:flex-row justify-between sm:items-center bg-gray-50/50 hover:bg-gray-50 transition focus:outline-none cursor-pointer">

                    <div class="flex items-center gap-4 mb-2 sm:mb-0">

                    <span class="bg-brand-dark text-white w-10 h-10 rounded-xl shadow flex items-center justify-center font-bold text-sm border border-gray-700">
