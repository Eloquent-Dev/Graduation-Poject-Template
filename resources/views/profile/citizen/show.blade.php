...
<div  class="grid grid-cols-1 md:grid-cols-3 gap-8">
           ...
           <a href="{{ route('citizen.profile.edit') }}" class="text-sm text-brand-blue hover:text-blue-800 font-medium transition">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 flex items-center justify-between">
                    ...
                    <button type="button" id="openPasswordBtn" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-lg transition text-sm">
                        Change Password
                    </button>
                </div>
            </div>
 ...
 <div id="passwordModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 sm:p-6">
        <div class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity opacity-0" id="modalBackdrop"></div>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md  overflow-hidden z-10 transition scale-95 opacity-0 transition-all duration-300" id="modalContent">
...
<i class="fa-solid fa-lock text-brand-blue"></i> Update Password
                <button type="button" id="closePasswordBtn" class="text-gray-400 hover:text-gray-400 hover:text-red-500 transition">
<i class="fa-solid fa-xmark"></i>
            ...
            <form action="{{ route('citizen.profile.password.update') }}" method="POST" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="password" name="current_password" class="w-full rounded-xl border border-gray-300 shadow-sm focus:border-brand-blue focus:ring focus:ring-2 focus:ring-brand-blue/20 px-4 py-2.5">
                    @error('current_password') <span class="text-xs text-red-500 font-bold mt-1 block">{{ $message }} </span>
                    @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">New Password</label>
                    <input type="password" name="new_password" class="w-full rounded-xl border border-gray-300 shadow-sm focus:border-brand-blue focus:ring focus:ring-2 focus:ring-brand-blue/20 px-4 py-2.5">
                ...
                <div>
                    ...
                    <input type="password" name="new_password_confirmation" class="w-full rounded-xl border border-gray-300 shadow-sm focus:border-brand-blue focus:ring focus:ring-2 focus:ring-brand-blue/20 px-4 py-2.5">
                </div>
                <div class="pt-4 flex justify-end gap-3">
                    
                    <button type="submit" class="px-6 py-2.5 bg-brand-blue hover:bg-blue-800 text-white font-bold rounded-xl shadow-md transition flex items-center gap-2">
    ...
    <script>
        ...
        @if ($errors->has('current-password') || $errors->has('password'))
            openPasswordModal();
